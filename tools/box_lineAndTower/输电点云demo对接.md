...
按照z轴从大到小排序，同时设定2m作为一个误差，即两根导线z的范围在2m之内，即视为同层导线，同层导线，比较全局x的大小，从小到大编号。
--conductor-layer-z-gap 2.0 # 参数说明：高度差才2m之内是为视为不同层。
**z轴怎么取**
导线拟合曲线在档距中点位置的 Z 值，中点的z轴
**x轴怎么取**
X 也是取这根导线在档距中点位置的全局 X 坐标，中点的x轴

**整体的排序规则**

1. 找到这根导线拟合曲线的档距中点 along （长）= mid
2. 计算这个中点处的 side 】
3. 计算这个中点处的 z
4. 把局部坐标 [along, side, z] 转回 LAS 全局坐标
5. 取全局 X 作为 sort_x
