"""清理杆塔误检，并为分割后的 LAS 追加杆塔框、档距框和拟合导线。

输入通常是 tools/infer_las_semseg.py 生成的 LAS：

    classification 0=background, 1=tower, 2=line, 3=insulator

脚本主要流程：
1. 将预测为杆塔的点聚类成三维连通组件；
2. 删除点数、高度、形态或线路关系不满足要求的杆塔误检；
3. 为保留的杆塔追加杆塔框边线点；
4. 沿主线路方向排序杆塔，并为相邻杆塔之间的导线生成档距框；
5. 在每个档距内拟合、编号并着色每根导线。

输出 LAS 会保留原始点，同时追加 class 30/31 的可视化点。
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Do not append or export between-tower line-span boxes. "
            "Conductor fitting and coloring are still performed."
        ),
    )
    parser.add_argument(
        "--line-tower-gap",
...
def estimate_span_z_slope(along, z, bin_size):
    """估计整档导线的总体高度坡度，用于陡坡档距中的稳定轨迹连接。"""
    if along.size < 2:
        return 0.0
    step = max(float(bin_size), 1e-3)
    along_min = float(along.min())
    bin_id = np.floor((along - along_min) / step).astype(np.int64)
    centers = []
    heights = []
    for item in np.unique(bin_id):
        ...
        if np.count_nonzero(mask) < 2:
            continue
        centers.append(float(np.median(along[mask])))
        heights.append(float(np.median(z[mask])))
    if len(centers) < 2:
        return 0.0
    slope = float(np.polyfit(np.asarray(centers), np.asarray(heights), 1)[0])
    if not np.isfinite(slope):
        return 0.0
    # 极端斜率通常来自异常点或错误档距，限制后可避免过度补偿。
...
def fit_conductors_for_spans(las, coord, span_boxes, args):
    ...
    for span in [box for box in span_boxes if box["kind"] == "line_span"]:
        ...
        # 对山地或大高差档距，直接比较相邻分箱的绝对 Z 会让同一根导线
        # 因整体坡度超过跟踪半径而断开。先消除整档线性坡度完成轨迹连接，
        # 轨迹建立后再恢复原始高度，最终拟合和输出仍使用 LAS 原坐标。
        slope_origin = float(np.median(along))
        span_z_slope = estimate_span_z_slope(
            along, side_z[:, 1], args.conductor_bin_size
        )
        tracking_side_z = side_z.copy()
        tracking_side_z[:, 1] -= span_z_slope * (along - slope_origin)
        ...
        for track in tracks:
            for sample in track["samples"]:
                sample["z"] += span_z_slope * (sample["along"] - slope_origin)
...
def main():
    ...
    # 步骤 0：如果输入已经做过后处理，先删除旧的可视化点。
    # 这样可以避免旧框、旧拟合导线和旧编号标签混入新结果。
    ...
    # 步骤 1：给原始语义类别重新着色，方便直接可视化。
    # 这一步不会修改 classification，只更新 RGB。
    recolor_by_class(las, cls, args)

    # 步骤 2：聚类预测为杆塔的点。模型可能会把一个真实杆塔分成多个组件，
    # 也可能把植被/地物误分成杆塔，所以清理从 class 1 连通组件开始。
    t0 = time.perf_counter()
    components, tower_indices, point_comp = cluster_towers(coord, cls, args)

    # 步骤 3：执行组件级过滤。基础过滤会使用点数、高度、XY 范围、
    # 局部地面高度和附近导线点。
    mark_components_to_keep(components, coord, cls, args)

    # 步骤 4：将邻近的保留组件合并为物理杆塔，并恢复低矮底部碎片，
    # 保证每座真实杆塔最终只有一个框。
    seed_towers = merge_physical_towers(components, args)
    recovered_base_components = recover_tower_base_components(
        components, seed_towers, args
    ...
    # 步骤 5：物理杆塔关系验证。默认依据杆塔和导线关系删除误检；
    # 绝缘子关系只有显式传入对应开关时才参与过滤。
    physical_towers, initial_tower_boxes, removed_physical_towers = (
        filter_towers_by_line_through_box(
            components, physical_towers, initial_tower_boxes, coord, cls, args
        )
    ...
    # 步骤 6：把清理结果写回 LAS 的 classification 字段。
    # 被删除的杆塔误检点不会物理删除，而是从杆塔类别改成背景类别，
    # 这样可以保留原始点密度和几何结构。
    ...
    # 步骤 7：生成可绘制 OBB。杆塔框来自保留的物理杆塔；
    # 档距框来自相邻杆塔之间、走廊范围内的 class 2 导线点。
    kept_components = [comp for comp in components if comp["keep"]]
    tower_box_list = tower_boxes(
        physical_towers, coord, tower_indices, point_comp, args.tower_box_margin
    )
    tower_box_by_id = {
        int(box["tower_id"]): box for box in tower_box_list if box["kind"] == "tower"
    }
    span_boxes = line_span_boxes(coord, cls, physical_towers, tower_box_by_id, args)
    boxes = list(tower_box_list)
    if not args.no_line_span_boxes:
        boxes.extend(span_boxes)
    print(
        "Boxes: kept_components={}, physical_towers={}, line_span={}, "
        "line_span_drawn={}".format(
            len(kept_components),
            len(physical_towers),
            len(span_boxes),
            0 if args.no_line_span_boxes else len(span_boxes),
        ),
        flush=True,
    )

    # 步骤 8：在每个有效档距内拟合每根导线。拟合导线按局部高度层和
    # 局部 side 位置编号，并按高度层着色，使同层导线颜色一致。
    (
        conductor_reports,
        conductor_points,
        conductor_colors,
    ...
    # 步骤 9：追加仅用于可视化的几何点。类别 30 保存拟合导线和编号标签；
    # 类别 31 保存杆塔框和档距框边线。
    appended_points = 0
    ...
    # 步骤 10：写出机器可读报告。主 report 用于调试；
    # *_boxes.json 用于绘制杆塔/档距 OBB；
    # *_conductors.json 用于渲染拟合导线折线。
    final_counts = {
        str(i): int(v) for i, v in enumerate(np.bincount(cls, minlength=32)) if v
    }
    ...
    line_box_report = make_standard_line_boxes(
        [] if args.no_line_span_boxes else span_boxes,
        obb_origin,
        args.json_box_orientation,
    )
...
""" 
python tools/box_lineAndTower/postprocess_tower_line_boxes.py \
  --input /24085403037/24085403037/shixi/dataset/lidar_tower_line_boxes/cloud0_clean.las \
  --output /24085403037/24085403037/shixi/dataset/lidar_tower_line_boxes/cloud0_postprocessed.las \
  --overwrite 
  """
