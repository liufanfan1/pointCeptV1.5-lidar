#!/usr/bin/env python
"""把输电线路预处理后的 .pth 数据 tile 导出成 PLY。

用途：
    检查数据本身是否正确，例如坐标、RGB、语义标签、tile 切分和
    world-coord 还原是否符合预期。它不读取模型预测。
输入：
    data/transmission_line*/{train,val,test} 下的 .pth tile。
输出：
    按标签色或原始 RGB 着色的 PLY；可选合并成一个 PLY。
"""
