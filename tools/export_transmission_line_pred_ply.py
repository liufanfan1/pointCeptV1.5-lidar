#!/usr/bin/env python
"""把输电线路通用预测结果 *_pred.npy 导出成 PLY。

用途：
    用于一阶段 4 类模型或最终 6 类预测的可视化。脚本会把预测类别、真值
    标签、原始 RGB 或 correct/wrong 映射成点云颜色。
输入：
    Pointcept 测试/推理生成的 *_pred.npy，以及对应 split 下同名 .pth。
输出：
    每个 tile 一个 PLY；可选合并成一个 PLY。
"""
