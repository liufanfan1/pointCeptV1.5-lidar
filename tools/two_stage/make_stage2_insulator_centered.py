"""生成以绝缘子为中心的 Stage-2 ROI 增强数据集。

用途：
    在已有 Stage-2 基础数据上，额外从原始 6 类数据中裁剪绝缘子附近的局部
    ROI，强化 Stage-2 对 insulator 的学习。适合绝缘子点数少、容易被 tower
    或 background 吃掉的情况。
输入：
    原始 6 类数据 data/transmission_line，以及已有 Stage-2 数据。
输出：
    默认 data/transmission_line_stage2_tower_ins_centered。
