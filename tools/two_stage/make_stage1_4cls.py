"""从原始 6 类输电线路数据生成 Stage-1 粗分 4 类数据集。

用途：
    两阶段流程的第一步数据准备。Stage-1 不直接区分 tower、insulator、
    hengdan，而是把它们合并成 tower_structure，目标是先高召回地找到杆塔
    相关区域，供 Stage-2 继续精分。
输入：
    默认 data/transmission_line，标签为原始 6 类。
输出：
    默认 data/transmission_line_stage1_4cls，标签为 Stage-1 4 类。
