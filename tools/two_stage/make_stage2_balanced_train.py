"""生成 Stage-2 ROI 4 类训练集的过采样/均衡版本。

用途：
    对 Stage-2 的 tower、insulator、hengdan、background ROI 数据做 train
    级别复制增强，重点提高 insulator 和 hengdan 的出现频率。val/test 不
    增强，保持真实分布用于评估。
输入：
    默认 data/transmission_line_stage2_tower。
输出：
    默认 data/transmission_line_stage2_tower_balance。
"""
