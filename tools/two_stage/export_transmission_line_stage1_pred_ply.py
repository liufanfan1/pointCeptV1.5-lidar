#!/usr/bin/env python
"""把两阶段流程中 Stage-1 粗分 4 类预测导出成 PLY。

用途：
    专门查看 Stage-1 是否把 tower/insulator/hengdan 统一召回为
    tower_structure。Stage-1 重点看杆塔结构召回率，因为 Stage-2 只会在
    Stage-1 找到的 ROI 附近继续细分。
输入：
    Stage-1 result 目录中的 *_pred.npy 和 Stage-1 4 类数据 .pth。
输出：
    4 类彩色 PLY：
        0 ground            灰色
