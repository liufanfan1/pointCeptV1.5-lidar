...
/opt/conda/envs/pointcept/bin/python \
  ...
  --voxel-size 0 # 体素降采样
  --tile-size 40 # 把每个场景按 XY 平面切成 40m x 40m 的瓦片
  --train-tile-stride 20 # 训练集切块步长是 20m，所以训练瓦片之间有重叠，增强训练样本数量。
  --eval-tile-stride 40 # 验证集和测试集切块步长是 40m，基本不重叠，评估更规整。
  --min-points 1024 # 少于 1024 个点的瓦片丢掉，避免太稀疏的 tile 影响训练或报错。

  /opt/conda/envs/pointcept/bin/python \
  pointcept/datasets/preprocessing/transmission_line/preprocess_transmission_line.py \
  --dataset-root /24085403037/24085403037/shixi/dataset/0617-4Name \
  --output-root data/transmission_line_stage1_random \
  --voxel-size 0 \
  --tile-size 40 \
  --train-tile-stride 20 \
  --eval-tile-stride 40 \
...
### tiles的理解：把一整个场景点云按 XY 平面切成很多个小块，每个小块单独保存成一个 .pth 训练样本。
为啥这样设置呢：
显存限制
一整个场景几百万点，PTv3 不能直接一次喂进去。切成 tile 后，每个样本点数可控。

增加训练样本数量
一个场景可以变成几十个 tile，训练更稳定。

保留局部几何上下文
比如导线、杆塔、绝缘子，本身判断依赖周围结构。40m tile 会给模型一个局部空间窗口。
...
/opt/conda/envs/pointcept/bin/python tools/make_stage1_4cls.py \
  --src-root data/transmission_line_stage1_random \
  --dst-root data/transmission_line_stage1_4cls_random \
  --overwrite
...
/opt/conda/envs/pointcept/bin/python tools/make_stage2_tower_roi.py \
  --src-root data/transmission_line_random \
...
## 四阶段
/opt/conda/envs/pointcept/bin/python tools/train.py   --config-file configs/transmission_line/semseg-pt-v3m1-4cls-ins-oversample.py   --num-gpus 1