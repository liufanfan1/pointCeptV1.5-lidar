...
@TRANSFORMS.register_module()
class ClassBalancedSphereCrop(object):
    def __init__(
        self,
        point_max=80000,
        sample_rate=None,
        ...
        self.mode = mode
        self.class_choices = np.array(class_choices, dtype=np.int64)
        self.class_probs = np.array(class_probs, dtype=np.float64)
    ...
    def __call__(self, data_dict):
        ...
        if segment is not None and np.random.rand() < self.p:
            order = np.random.choice(
                len(self.class_choices),
                size=len(self.class_choices),
                replace=False,
                p=self.class_probs,
            )
            for choice_idx in order:
                class_id = self.class_choices[choice_idx]
                candidate = np.flatnonzero(segment == class_id)
                if candidate.size > 0:
                    center_idx = np.random.choice(candidate)
